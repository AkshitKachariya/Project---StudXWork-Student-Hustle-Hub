...
export default function RecruiterNotices() {
  const [notices, setNotices] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    api.get('/notices?role=recruiter').then(res => setNotices(res.data.data || [])).finally(() => setLoading(false));
  }, []);

  return (
    <div>
      <div className="mb-6">
        <div className="neo-badge bg-brand-secondary text-white mb-2">Announcements</div>
      ...
      {loading ? <div className="neo-card p-8 animate-pulse h-24 bg-gray-100" /> :
       notices.length === 0 ? (
        ...
        <div className="space-y-4">
          {notices.map((n) => (
            <div key={n._id} className="neo-card p-6 border-l-5 border-l-brand-secondary">
