...
const STATUS_CONFIG = {
  ...
  assigned:    { color: 'bg-indigo-100 text-indigo-800', icon: CheckCircle, label: 'Assigned' },
...
export default function TaskWorkspace() {
  ...
  const handleSubmit = async () => {
    if (!fileUrls.length && !links.length && !message.trim()) {
      return toast.error('Please add file URLs, links, or a message');
    }
    setSubmitting(true);
    try {
      await api.post(`/workflow/submit/${taskId}`, {
        fileUrls,
        links,
        message
      });
  ...
  const canSubmit = task.status === 'in_progress' || task.status === 'revision';
  ...
  return (
    <div className="max-w-6xl mx-auto">
          ...
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              {task.revisionCount > 0 && (
                <span className="neo-badge bg-amber-100 text-amber-800">
            ...
            <p className="text-brand-muted font-medium mt-1">
              Posted by <span className="font-bold text-brand-dark">{task.recruiter?.companyName || task.recruiter?.name}</span>
            </p>
          </div>
          <div className="flex items-center gap-3">
            <div className="text-right">
              <p className="text-xs font-bold text-brand-muted uppercase">Budget</p>
          ...
          {WORKFLOW_STEPS.map((step, i) => (
            <div key={step} className="flex-1 flex items-center">
              <div className={`h-2 flex-1 border-2 border-brand-dark transition-all ${
                i <= currentStep ? 'bg-brand-success' : 
                task.status === 'rejected' ? 'bg-red-200' : 'bg-gray-200'
              }`} />
            </div>
          ))}
        </div>
      ...
      {/* Revision Alert */}
      {task.status === 'revision' && task.revisionFeedback && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          className="neo-card p-5 mb-6 bg-amber-50 border-amber-400 border-l-4">
          <div className="flex items-start gap-3">
            <AlertTriangle className="text-amber-600 shrink-0 mt-0.5" size={20} />
            <div>
              <h3 className="font-black uppercase text-amber-800">Revision Requested</h3>
              <p className="text-sm font-medium text-amber-700 mt-1">{task.revisionFeedback}</p>
            </div>
          </div>
        </motion.div>
      )}
      ...
      {task.status === 'rejected' && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          className="neo-card p-5 mb-6 bg-red-50 border-red-400 border-l-4">
          <div className="flex items-start gap-3">
      ...
      {task.status === 'completed' && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          className="neo-card p-5 mb-6 bg-green-50 border-green-400 border-l-4">
          <div className="flex items-start gap-3">
            <CheckCircle className="text-green-600 shrink-0 mt-0.5" size={20} />
            <div>
              <h3 className="font-black uppercase text-green-800">Task Completed! 🎉</h3>
              <p className="text-sm font-medium text-green-700 mt-1">₹{task.escrowAmount} has been credited to your wallet.</p>
            </div>
          </div>
        </motion.div>
      )}
          ...
          <motion.div key="overview" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="space-y-4">
                ...
                <button onClick={() => navigate(role === 'student' ? '/student/chat' : '/recruiter/chat', { state: { recruiter: role === 'student' ? task.recruiter : task.assignedTo } })}
                  className="neo-btn-primary w-full flex items-center justify-center gap-2">
                  <MessageCircle size={18} /> Chat with {role === 'student' ? 'Recruiter' : 'Student'}
                </button>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div key="submissions" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            {(!task.submissions || task.submissions.length === 0) ? (
              <div className="neo-card p-12 text-center">
                ...
                <p className="text-brand-muted mt-2">Start working and submit your deliverables.</p>
              </div>
            ) : (
              <div className="space-y-4">
                {[...task.submissions].reverse().map((sub, i) => (
                  <motion.div key={i} whileHover={{ x: 4 }} className="neo-card p-6">
                    ...
                    {sub.files?.length > 0 && (
                      <div className="mb-3">
                        <p className="text-xs font-bold uppercase text-brand-muted mb-2">Files</p>
                        <div className="flex flex-wrap gap-2">
                          {sub.files.map((f, fi) => (
          ...
          <motion.div key="submit" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <div className="neo-card p-8">
              <h3 className="text-xl font-black uppercase mb-6">
                {task.status === 'revision' ? '🔁 Resubmit Work' : '📤 Submit Your Work'}
              </h3>

              {/* File URLs */}
              <div className="mb-6">
                <label className="block font-bold mb-2">File URLs (paste online links to files, images, docs, etc.)</label>
                <div className="flex gap-2">
                  <input value={fileNameInput} onChange={(e) => setFileNameInput(e.target.value)} placeholder="File name (e.g. Design.pdf)" className="neo-input w-44" />
                  <input value={fileUrlInput} onChange={(e) => setFileUrlInput(e.target.value)} placeholder="https://drive.google.com/..." className="neo-input flex-1" 
                  ...
                  <div className="flex flex-wrap gap-2 mt-3">
                    {fileUrls.map((f, i) => (
                      <span key={i} className="neo-badge bg-brand-bg flex items-center gap-2">
                        ...
                        <button onClick={() => setFileUrls(fileUrls.filter((_, fi) => fi !== i))} className="text-red-500 hover:text-red-700">
                          <X size={12} />
                        </button>
                      </span>
                    ))}
              ...
              {/* Links */}
              <div className="mb-6">
                <label className="block font-bold mb-2">External Links</label>
                <div className="flex gap-2">
                  <input value={linkLabel} onChange={(e) => setLinkLabel(e.target.value)} placeholder="Label (e.g. GitHub)" className="neo-input w-40" />
                  ...
                  <div className="flex flex-wrap gap-2 mt-3">
                    {links.map((l, i) => (
                      <span key={i} className="neo-badge bg-brand-primary/10 text-brand-primary flex items-center gap-2">
                        ...
                        <button onClick={() => setLinks(links.filter((_, li) => li !== i))} className="text-red-500 hover:text-red-700">
                          <X size={12} />
                        </button>
                      </span>
                    ))}
              ...
              <div className="mb-6">
                ...
                <textarea value={message} onChange={(e) => setMessage(e.target.value)} rows={4}
                  className="neo-input resize-none" placeholder="Describe what you've done, any notes for the recruiter..." />
              </div>

              <button onClick={handleSubmit} disabled={submitting} className="neo-btn-primary w-full py-4 text-lg">
